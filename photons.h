...
public:
     ...
     */
    double move(double (*mu_s)(double vect)) {
        double rnum = 0;
        do { rnum = rand_num(); } while (rnum == 0);
        double deltaS = -log(rnum) / (mu_a+mu_s(this->S.Q));
        x += U.i * deltaS;
        y += U.j * deltaS;
        z += U.k * deltaS;
        return deltaS;
    ...
    void rejection(void) {
        ...
        do {
            ...
            phi = 2*M_PI*rand_num();
            Io = mie_props->s11[0]*S.I + mie_props->s12[0]*(S.Q*cos(2*phi) + S.U*sin(2*phi));
            int i = floor(theta / M_PI * nangles);
            Iith = mie_props->s11[i]*S.I + mie_props->s12[i]*(S.Q*cos(2*phi) + S.U*sin(2*phi));
            rand_no = rand_num();
    ...
    void quaternion_scatter(void) {
        ...
        S.I= mie_props->s11[ithedeg]*si+mie_props->s12[ithedeg]*sq;
        S.Q= mie_props->s12[ithedeg]*si+mie_props->s11[ithedeg]*sq;
        S.U= mie_props->s33[ithedeg]*su+mie_props->s43[ithedeg]*sv;
        S.V= -mie_props->s43[ithedeg]*su+mie_props->s33[ithedeg]*sv;
        S.normalize();
    }
};
